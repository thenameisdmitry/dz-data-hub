...
system to the moment it is available in the database. Together with
...
- **Excluded:** `rejected` transactions (they never reach the
  database, so they have no end-to-end latency) and test feeds.
...
SELECT
  DATE_TRUNC('month', ingested_at)     AS month,
    ...
    -- DATEDIFF = minutes between arrival and DB availability
    ROUND(MEDIAN(
...
-- Only records that reached the DB have an end-to-end
...
transactions are typically 20–40 x slower than `auto`, since they wait
...
is set by the ingestion layer on arrival, `loaded_at` by the database
...
- **Update schedule:** daily database load at 03:00 UTC
